...
static void test_contention(skinny_mutex_t *mutex)
{
    ...
    struct test_contention tc = {.mutex = mutex, .held = false, .count = 0};
...
static void test_lock_cancellation(skinny_mutex_t *mutex)
{
    ...
    assert(!skinny_mutex_lock(mutex));
    assert(!pthread_create(&thread, NULL, lock_cancellation_thread, mutex));
    delay();
...
static void test_trylock(skinny_mutex_t *mutex)
{
    ...
    assert(!pthread_create(&thread1, NULL, trylock_contender_thread, mutex));
    delay();
...
{
    struct test_cond_wait tcw = {.mutex = mutex};
    pthread_t thread;
...
static void do_test_hammer(void (*f)(skinny_mutex_t *m))
{
    ...
    for (i = 0; i < 10; i++)
        assert(
            !pthread_create(&threads[i], NULL, do_test_hammer_thread, &mutex));
